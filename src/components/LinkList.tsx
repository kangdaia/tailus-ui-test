...
export const Link = ({ isActive = false, link, children }: { isActive?: boolean; children: ReactNode; link: string }) => {
  return (
    <Button.Root
      href={link}
...
export const Trigger = ({ children }: { children: ReactNode }) => (
  <Accordion.Header>
    <Accordion.Trigger className={button.ghost({ intent: 'gray', size: 'md', className: 'w-full justify-start gap-3.5 px-3 text-sm' })}>
      {children}
